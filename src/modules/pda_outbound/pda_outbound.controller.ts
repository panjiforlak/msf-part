...
export class PdaOutboundController {
  ...
  async findAll(@Request() req, @Query() query: PdaOutboundQueryDto) {
    const userId = req.user.id;
    const data = await this.pdaOutboundService.findAll(
      userId,
      query.superadmin,
    );
  ...
  @ApiOperation({
    ...
    description:
      'Mengambil data batch outbound berdasarkan order form ID. Jika parameter is_queue=true, hanya menampilkan data yang sudah masuk ke tabel relocation (flag_queue=true)',
  })
  @ApiParam({
    name: 'orderFormId',
  ...
  async findBatchOutboundByOrderFormId(
    ...
    const data = await this.pdaOutboundService.findBatchOutboundByOrderFormId(
      orderFormId,
      isQueueBoolean,
    );
  ...
  async createRelocation(
    ...
    const userId = req.user.id;
    const data = await this.pdaOutboundService.createRelocation(
      createRelocationDto,
      userId,
    );
  ...
  async scanDestination(
    ...
    const userId = req.user.id;
    const data = await this.pdaOutboundService.scanDestination(
      scanDestinationDto,
      userId,
    );
  ...
  async getAreaOutbound(@Query() getAreaOutboundDto: GetAreaOutboundDto) {
    const data =
      await this.pdaOutboundService.getAreaOutbound(getAreaOutboundDto);
  ...
  async getRelocationHistory(@Query() query: RelocationHistoryQueryDto) {
